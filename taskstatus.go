...
type taskStatusUpdate struct {
	Status     TaskStatus
	Function   func(task *TaskRun) error
...
func (tsm *TaskStatusManager) updateStatus(ts TaskStatus, f func(task *TaskRun) error, fromStatuses ...TaskStatus) error {
	m := make(map[TaskStatus]bool, len(fromStatuses))
...
func NewTaskStatusManager(task *TaskRun) *TaskStatusManager {
	...
	go func() {
		for update := range r {
			if len(update.IfStatusIn) == 0 || update.IfStatusIn[task.Status] {
				task.Status = update.Status
				e <- update.Function(task)
			} else {
