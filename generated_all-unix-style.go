...
type (
	...
	GenericWorkerPayload struct {
		...
		Features struct {
			...
			// Normally, tasks are sandboxed by running the task commands as
			// a one-time user, generated by the generic-worker. When enabling
			// this feature, the task commands will be executed as the same user
			// that runs the generic-worker. On Windows, this means executing
			// in the context of the LocalSystem account, running as a Windows
			// service. Typically this is only required for generic-worker CI
			// tests, since generic-worker needs to run from this context, to be
			// able to create task users and interact with them. Requires the
			// scope `generic-worker:runAsWorkerUser:<workerType>`.
  ...
  "properties": {
    ...
    "features": {
      ...
      "properties": {
        ...
        "runAsWorkerUser": {
          "description": "Normally, tasks are sandboxed by running the task commands as\na one-time user, generated by the generic-worker. When enabling\nthis feature, the task commands will be executed as the same user\nthat runs the generic-worker. On Windows, this means executing\nin the context of the LocalSystem account, running as a Windows\nservice. Typically this is only required for generic-worker CI\ntests, since generic-worker needs to run from this context, to be\nable to create task users and interact with them. Requires the\nscope ` + "`" + `generic-worker:runAsWorkerUser:\u003cworkerType\u003e` + "`" + `.",
          "title": "Task commands to be executed as the user running generic-worker",
          "type": "boolean"
        }
      },
      "title": "Feature flags",
